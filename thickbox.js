	...
	try {
		...
		if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp' || urlType == '.webp'){//code to show images
			...
			imgPreloader.onload = function(){
				...
				tb_fitWidth = x;
				tb_fitHeight = Math.round(imageHeight * (x / imageWidth));
				if (tb_fitHeight > y) {
					tb_fitWidth = Math.round(tb_fitWidth * (y / tb_fitHeight));
					tb_fitHeight = y;
				}
				tb_fitToWindow = tb_options.auto_resize && (imageWidth > x || imageHeight > y);
				if (tb_fitToWindow) {
					imageWidth = tb_fitWidth;
					imageHeight = tb_fitHeight;
				}
				// End Resizing
				...
				else if (tb_options.click_img == "open")
					TB_Image = "<a href='"+url+"' target='_blank' title='"+thickboxL10n.open+"'>" + TB_Image + "</a>";
				else if (tb_options.click_img == "download")
					TB_Image = "<a href='"+url+"' class='download' title='"+thickboxL10n.download+"'>" + TB_Image + "</a>";
				else if (tb_options.click_img == "expand_shrink")
					TB_Image = "<a href='' class='scalable' title='"+(tb_fitToWindow ? thickboxL10n.actual : thickboxL10n.fit)+"'>" + TB_Image + "</a>";
				...
				if (tb_options.click_img == "download") {
					jQuery("#TB_window a.download").click(function() {
						window.location = thickboxL10n.forceDL + '?file=' + this.href;
						return false;
					});
				} else if (tb_options.click_img == "expand_shrink") {
					jQuery("#TB_window a.scalable").click(function() {
						if (tb_fitToWindow)
							setImageSize(imgPreloader.width, imgPreloader.height, thickboxL10n.fit);
						else
							setImageSize(tb_fitWidth, tb_fitHeight, thickboxL10n.actual);
						tb_fitToWindow = !tb_fitToWindow;
						return false;
					});
				}

				function setImageSize(width, height, title) {
					var diffWidth = width - jQuery("#TB_Image").width();
					var diffHeight = height - jQuery("#TB_Image").height();
					jQuery("#TB_window").css({
						marginLeft: Math.round(parseInt(jQuery("#TB_window").css("marginLeft")) - (diffWidth / 2)),
						marginTop: Math.round(parseInt(jQuery("#TB_window").css("marginTop")) - (diffHeight / 2))
					});
					jQuery("#TB_window").width(jQuery("#TB_window").width() + diffWidth).height('auto');
					jQuery("#TB_Image").width(width).height(height);
					if (title)
						jQuery("#TB_window a.scalable").attr("title", title);
				}
				...
				if (imageGroup && tb_options.wheel_img == "prev_next") {
					jQuery("#TB_window").bind('mousewheel.thickbox DOMMouseScroll.thickbox', function(e){
						e.stopImmediatePropagation();
						...
						// workaround for a issue that page scroll occurs in Firefox when the handler returns false
						jQuery(window).one('MozMousePixelScroll', function() { return false; });
				...
				} else if (tb_options.wheel_img == "scale") {
					jQuery("#TB_window").bind('mousewheel.thickbox DOMMouseScroll.thickbox', function(e){
						...
						var width = Math.round(jQuery("#TB_Image").width() * scale);
						var height = Math.round(jQuery("#TB_Image").height() * scale);
						if (width > 100 && height > 100) // minimum size (100x100)
							setImageSize(width, height);
						return false;
					});
				} else if (tb_options.wheel_img == "none") {
					jQuery("#TB_window").bind('mousewheel.thickbox DOMMouseScroll.thickbox', function(e){
						e.stopImmediatePropagation();
						return false;
					});
				...
				if (tb_options.wheel_bg == "none") {
					jQuery("#TB_overlay").bind('mousewheel.thickbox DOMMouseScroll.thickbox', function(e){
						e.stopImmediatePropagation();
						return false;
					});
				}
			...
			TB_WIDTH = (params['width']*1) || tb_options.win_width; //defaults to 600 if no paramaters were added to URL
			TB_HEIGHT = (params['height']*1) || tb_options.win_height; //defaults to 400 if no paramaters were added to URL
...
function tb_drag(event, source, action) {
	...
	var keepRatio = action == "ne-resize" || action == "nw-resize" || action == "se-resize" || action == "sw-resize";
	if (keepRatio)
		var ratio = jQuery("#TB_window").width() / jQuery("#TB_window").height();
	if (action != "move") {
	...
	jQuery(document).bind("mousemove.thickbox", function (e, clientX, clientY) {
		...
		if (startDrag) {
			if (keepRatio) {
				...
				if (ratio > newRatio)
					offsetY = Math.round(offsetX / ratio) * signX * signY;
				else if (ratio < newRatio)
					offsetX = Math.round(offsetY * ratio) * signX * signY;
			}
